...
当前方案的全量 DELETE 是幂等性的基础，但代价是任何一批失败都要重跑全量。要实现"不删其他批次"，核心思路是**将全量幂等降级为批次级幂等，同时引入断点续传**，需要两处改造。

---

**改造一：Consumer 侧从"全局 DELETE + INSERT IGNORE"改为"批次级 DELETE + INSERT"**

MQ 消息 `FaultDataBatchMessage` 本身携带了该批次的全部记录，可以从中取出 `minRank` 和 `maxRank`。Consumer 消费时：

```java
// 1. 清除本批次 rank 范围内的旧数据（只影响自己，不动其他批次）
faultRecordMapper.deleteByRankRange(domain, dataDate, minRank, maxRank);

// 2. 写入本批次最新数据
faultRecordMapper.batchInsert(entities);   // INSERT INTO（非 IGNORE，因为旧数据已删）
```

两步操作用 `@Transactional` 包裹保证原子性：DELETE 和 INSERT 要么同时成功，要么同时回滚，不会产生该 rank 范围的空窗口。

每条 MQ 消息的处理独立幂等：重复消费同一条消息，DELETE 清掉该 rank 范围的数据，INSERT 重新写入，结果等价于只消费一次。

---

**改造二：sync_task_record 增加 last\_successful\_rank 字段，支持拉取断点续传**

```sql
ALTER TABLE sync_task_record ADD COLUMN last_successful_rank INT DEFAULT 0;
```

生产侧 pull 循环中，每批 pull + MQ send 成功后持久化进度：

```sql
UPDATE sync_task_record
SET last_successful_rank = #{lastRank}
WHERE domain = #{domain} AND data_date = #{dataDate};
```

当 pull 第 N 批报错时，`last_successful_rank` 保存的是第 N-1 批的最大 rank。PowerJob 重跑时：

- **不再执行全局 DELETE**，前 N-1 批的数据继续保留
- pull 循环从 `last_successful_rank + 1` 开始，只拉取未成功的批次
- Consumer 对重新发出的批次做批次级 DELETE + INSERT，幂等覆盖

---

**改造三：消费侧入库失败的补偿路径**

MQ 重试 3 次仍失败进 DLQ 后，当前方案直接标 FAILED。新方案中 DLQ Consumer 改为：

1. 记录失败批次的 `domain`、`data_date`、`minRank`、`maxRank` 到 `sync_batch_record` 补偿表
2. 触发告警
3. 由修复任务定期扫描 `sync_batch_record` 中 status=FAILED 的行，重新 pull 对应 rank 范围并发 MQ，Consumer 批次级幂等处理

---

**改造前后对比**

| 维度 | 当前方案（全量重跑） | 新方案（批次级重跑） |
|------|--------------|--------------|
| 失败重跑代价 | 重拉所有批次（顶峰 200 批） | 只补跑失败批次（1 批） |
| 已成功批次数据 | 全部 DELETE 重写 | 保留，不影响 |
| 实现复杂度 | 低 | 中（需 last_successful_rank + 补偿表） |
| 上游数据修改兼容 | 全量覆盖，完全兼容 | 跳过已同步批次，修改过的数据不会被重拉 |
| Consumer 事务要求 | 无（INSERT IGNORE 可接受重复） | 需要 @Transactional 保证 DELETE+INSERT 原子 |
| 适用场景 | 数据量适中 / 对完整性要求高 | 顶峰场景 / 上游数据 append-only |

---

**关键权衡**

引入断点续传的最大代价是：**对于已成功拉取的批次，若上游在该 rank 范围内修改了记录，本次重跑不会感知到这些修改**。

- 若业务保证上游数据 rank 一旦产生不会修改（只新增），新方案完全安全
- 若上游数据有修改可能，断点续传只适合应急补跑，最终还是要走一次全量同步修复

实际工程中的选择：**顶峰场景先用新方案补跑减少重跑代价，补跑完后视情况决定是否发起一次全量修复**。
...
Consumer 侧的失败不能靠标量字段，需要一张按批次独立记录状态的表：
...
DLQ Consumer 收到批次 2 的死信后，将 `(domain, data_date, batch_index=2, minRank, maxRank, status=FAILED)` 写入该表。补跑任务扫描 `status=FAILED` 的行，重新 pull 对应 rank 范围并重新发 MQ，Consumer 以批次级 DELETE + INSERT 幂等处理。批次 3 的成功与批次 2 的失败完全独立记录，互不干扰。
...
DLQ Consumer 收到 batch_2 的死信消息
    ├── 消息里有 records（rank 10001~15000 的完整数据）
    ├── 执行批次级 DELETE + INSERT（WHERE rank BETWEEN minRank AND maxRank）
    ├── 成功 → 更新 sync_batch_record 为 SUCCESS，completedBatchCount +1 → SUCCESS ✓
    └── 失败（DB 持续异常）→ 写入 sync_batch_record(status=FAILED) 留待补跑，触发告警
```

**第二层：修复任务用 minRank/maxRank 定向重拉（DLQ 重试也失败时）**

`sync_batch_record` 记录了 `min_rank` 和 `max_rank`，修复任务利用这两个值从上游拉取该 rank 范围内的数据：

```java
// 查出 FAILED 行：minRank=10001, maxRank=15000
List<FaultRecordDTO> records = client.pull(domain, date, minRank - 1, pageSize);
records = records.stream()
                 .filter(r -> r.getRank() <= maxRank)
                 .collect(toList());

// 事务内：只操作这个 rank 范围
faultRecordMapper.deleteByRankRange(domain, date, minRank, maxRank);
faultRecordMapper.batchInsert(convert(records));

syncBatchRecordMapper.markSuccess(id);
// 检查所有批次均完成 → 置 sync_task_record 为 SUCCESS
```

批次 1、2、4、5 的数据完全不受影响，因为 DELETE 和 INSERT 都限定了 `rank BETWEEN minRank AND maxRank`。

**完整数据流示例**

```
正常发送（5批）：
  Producer: batch_0(rank 1~5000) → batch_1(5001~10000) → batch_2(10001~15000)
           → batch_3(15001~20000) → batch_4(20001~22000)
  updateMessagesSent(batchCount=5) + checkAndMarkSuccessIfAllDone

消费阶段：
  batch_0 → OK  completedBatchCount=1
  batch_1 → OK  completedBatchCount=2
  batch_2 → 失败×3 → DLQ
  batch_3 → OK  completedBatchCount=3
  batch_4 → OK  completedBatchCount=4

DLQ Consumer：
  ├── 尝试直接从消息数据重写 batch_2（批次级 DELETE + INSERT）
  │     成功 → completedBatchCount=5 → SUCCESS ✓
  │     失败 → sync_batch_record(minRank=10001, maxRank=15000, FAILED)
  │            sync_task_record → FAILED，触发告警

修复任务（扫描 sync_batch_record FAILED 行）：
  pull(domain, date, lastRank=10000, 5000) → rank 10001~15000
  DELETE fault_record WHERE rank BETWEEN 10001 AND 15000
  INSERT 重拉数据
  sync_batch_record → SUCCESS
  所有批次完成 → sync_task_record → SUCCESS ✓
```
...
| 修复时效 | 等 PowerJob 重试窗口 | DLQ Consumer 可立即尝试 |
