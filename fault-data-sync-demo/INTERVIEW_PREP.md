...
```
           ...
           │  │                                                      │  │
           │  │  b. syncBatchRecordService.markPullSuccess(          │  │
           │  │       batchIndex, startRank, endRank, size)          │  │
           │  │                                                      │  │
           │  │  c. FaultDataProducer.syncSend(                      │  │
           │  │       FaultDataBatchMessage{domain, date,            │  │
           │  │         batchIndex, startRank, records})             │  │
           │  │     → RocketMQ: fault-data-sync-topic                │  │
           │  │       发送失败自动重试 2 次                           │  │
           │  │                                                      │  │
           │  │  d. lastRank = max(records[*].rank)                  │  │
           │  │                                                      │  │
           │  │  e. records.size() < 5000  →  EXIT（最后一批）       │  │
           │  │     else  →  继续拉取下一批                          │  │
           │  └──────────────────────────────────────────────────────┘  │
           │     正常场景：2万条 / 4批；顶峰：100万条 / 200批            │
           │                                                            │
           │  [4] sync_task_record  →  MESSAGES_SENT(batchCount=N)     │
           │  [5] checkAndMarkSuccessIfAllDone（Producer 补偿检查）     │
           └────────────────────────────────────────────────────────────┘
                                        │
                    [5 个 CF 全部完成，汇总结果]
                                        │
...
═══════════════════ 故障路径 A：拉取阶段报错 ═══════════════════
  ...
  例：共8批数据（batch 0~7），前3批（batch 0~2）拉取+发送成功，batch 3 client.pull() 抛异常

  runFirstSync 中：
    batch 0~2：markPullSuccess() ✓  →  sendBatch() ✓  →  sync_batch_record pull_status=SUCCESS
    batch 3：client.pull() 抛异常
          │
          ▼
    markPullFailed(domain, date, batchIndex=3, startRank, err)
    sync_task_record  →  FAILED（MESSAGES_SENT 未设置，batchCount 未记录）
    异常向上传播  →  CompletableFuture 返回 "FAIL:domain_a_D-1:连接超时"

  已发出的 batch 0~2 MQ 消息：
    Consumer 正常消费，markInsertSuccess() ✓，INSERT IGNORE 写库 ✓
    BUT: incrementCompletedBatch WHERE status IN ('RUNNING','MESSAGES_SENT') 不匹配
         （当前状态为 FAILED）→  0行更新  →  任务不会变 SUCCESS

  PowerJob 任务重试（runRetrySync）：
    [1] createOrUpdateRunning()  →  RUNNING（batchCount/completedBatchCount 重置为 0）
    [2] hasSuccessBatch() = true（batch 0~2 有 pull_status=SUCCESS 记录）→ runRetrySync()
    [3] findFailed() 返回 batch 3（pull_status=FAILED）
    [4] 从 batch3.startRank 开始 while 循环重拉，覆盖 batch 3~7（无 DELETE）
    [5] batch 0~2 数据继续保留，老 MQ 消息若再被消费 → INSERT IGNORE 跳过
    [6] batch 3~7 全部消费成功 → SUCCESS ✓
    ✅ 批次级续拉，不需要从头全量重跑


═══════════════════ 故障路径 B：消费侧入库失败 ═══════════════════

  例：batch 0~7 全部拉取+发送成功，batch 5（0-indexed）Consumer 写库失败

  pull 循环不感知消费侧结果：
    batch 0~7 全部发出，sync_batch_record 各批 pull_status=SUCCESS
    sync_task_record  →  MESSAGES_SENT(8)

  FaultDataConsumer 消费 batch 5 → batchInsert 抛异常
          │
          ▼
  RocketMQ 指数退避重投（最多 maxReconsumeTimes=3 次）
          │ 其余 7 批独立消费，markInsertSuccess() ✓，正常写库
          │ batch 5 重试 3 次仍失败
          ▼
  ...
  FaultDataDlqConsumer.onMessage()
    syncBatchRecordService.markInsertFailed(domain, date, batchIndex=5, err)
    syncTaskRecordService.updateFailed(domain, date, err)
    sync_task_record  →  FAILED，触发告警钩子（钉钉 / 邮件 / PagerDuty 可扩展）

  最终状态：fault_record 有 7 批数据（35000条），batch 5（5000条）缺失
            sync_batch_record：batch 5 insert_status=FAILED，其余 SUCCESS

  PowerJob 任务重跑（runRetrySync）：
    [1] hasSuccessBatch() = true → runRetrySync()
    [2] findFailed() 返回 batch 5（insert_status=FAILED）
    [3] 仅重拉 batch 5（单次 pull from startRank，无 DELETE）
    [4] sendBatch() → Consumer INSERT IGNORE 写入缺失的 5000 条
    [5] batch 5 消费成功 → completedBatchCount 追上 batchCount → SUCCESS ✓
    ✅ 只补跑一批，其余 7 批数据完全不受影响
...
═══════════════════ 两种故障的恢复层级对比 ═══════════════════
  ...
  │  sync_batch_record +         │  持久批次失败；仅重跑失败批次 │  最小：无 DELETE，      │
  │  runRetrySync                │                              │  不动已成功批次数据     │
  │  PowerJob 任务重跑           │  任何持久故障，下次触发时续跑 │  中：重拉失败批次       │
  │  首次运行 DELETE +           │  幂等兜底，全量重跑正确       │  短暂空窗口可接受       │
  │  INSERT IGNORE               │                              │                        │
  └──────────────────────────────┴──────────────────────────────┴────────────────────────┘
```
...
```sql
-- 放宽 WHERE 至 RUNNING/MESSAGES_SENT，修复消费者早于 updateMessagesSent 的竞态
-- SUCCESS 触发直接引用 DB 列 batch_count，加 batch_count > 0 守卫防 RUNNING 阶段误触发
UPDATE sync_task_record
SET completed_batch_count = completed_batch_count + 1,
    status = CASE
               WHEN completed_batch_count + 1 >= batch_count AND batch_count > 0 THEN 'SUCCESS'
               ELSE status
             END,
    end_time = CASE
                 WHEN completed_batch_count + 1 >= batch_count AND batch_count > 0 THEN NOW()
                 ELSE end_time
               END
WHERE domain = #{domain}
...
#### 补充 SQL：checkAndMarkSuccessIfAllDone（Producer 补偿）

```sql
-- Producer 在 updateMessagesSent 后立即执行，处理全部批次均早于该步骤消费完成的场景
UPDATE sync_task_record
...
**为什么这样设计：**
1. **原子性**：`completed_batch_count + 1` 在 DB 层计算，MySQL 行锁保证多个 Consumer 并发时串行化，无需应用层锁
2. **覆盖竞态**：WHERE 放宽到 `IN ('RUNNING', 'MESSAGES_SENT')`，Consumer 早到时在 RUNNING 阶段也能正常计数；`batch_count > 0` 守卫防止 RUNNING 阶段（batch_count=0）被误判完成
3. **Producer 补偿**：`checkAndMarkSuccessIfAllDone` 兜底"所有批次均早于 updateMessagesSent 消费完成"的极端场景
4. **防幽灵计数**：一旦状态变为 FAILED（DLQ 触发），`status IN (...)` 不再匹配，后续 Consumer 的 UPDATE 影响行数为 0
...
引入 MQ 有三个具体原因，其中第三点是最核心、逻辑链最完整的：

**原因一（最核心）：批次级重试，而非整任务重跑**

不加 MQ 的方案里，200 批数据由 Job 线程串行拉取并写库。假设第 180 批 INSERT 失败：
- 要么跳过（数据丢失）
- 要么 PowerJob 重跑整个任务，前 179 批数据重新拉取 + 写库，代价高，且上游接口有限流，大量重复调用带来额外压力

引入 MQ 后，每批 5k 数据封装成一条消息，消费失败时 RocketMQ 只重投这一条消息，其余 199 批不受影响。这是 MQ 解决的核心问题：**精确到批次级别的重试**。

**原因二（辅助）：上游接口有限流，拉取不应被写库阻塞**

上游是外部系统，有 QPS 限制。不加 MQ 时，拉取线程和写库操作串行在同一线程，DB 一旦抖动，拉取也随之停顿，占用接口调用时间窗口。解耦后，拉取线程只负责发消息，不等写库结果，调用节奏更稳定。

**原因三（辅助）：与其他业务共用 DB，峰值写入需要控制**

多个业务模块共用同一个 MySQL 实例，故障高峰期故障数据量大，集中写库会打高连接数，影响其他业务。Consumer 端可以通过控制并发数和批次大小，以 DB 能承受的速率匀速消费，避免写入峰值影响其他模块。

> 注意：原因二和三单独都不能直接推出"必须引入 MQ"——限流可以用令牌桶解决，写竞争可以限制线程池大小。只有原因一（批次级重试需求）的逻辑链是完整封闭的，是面试时最站得住脚的理由。

**引入 MQ 的代价：**
1. 增加了系统复杂度，需要维护 RocketMQ 集群
2. 引入最终一致性，数据写库有延迟（不是拉完立刻可查）
3. 消息序列化/反序列化有额外开销
4. 需要处理 MQ 故障场景（Broker 不可用时生产者怎么办）

这个场景是每日批处理、允许异步，引入 MQ 的收益远大于代价。如果是实时同步、要求秒级可见，就需要重新评估。
...
**Q6.1（追问）：Consumer 是否可能在 updateMessagesSent 之前就执行 incrementCompletedBatch？如果可以，会出现什么问题？怎么解决？**
...
Producer 的执行顺序是：`sendBatch(batch_0)` → ... → `sendBatch(batch_N-1)` → `updateMessagesSent(N)`。RocketMQ 是消息进 Broker 即返回，Consumer 和 Producer 是独立线程。第 0 批进入 Broker 后，Consumer 可能立刻消费它，此时 Producer 还在发第 1、2、3... 批，`updateMessagesSent` 根本还没执行。

原始代码中 `incrementCompletedBatch` 的 WHERE 条件为 `status='MESSAGES_SENT'`，此时 status 仍是 `RUNNING`，UPDATE 影响行数为 0。**这一批的计数被静默丢弃**，`completed_batch_count` 永远差几个，SUCCESS 永远触发不了。

**两种竞态场景**

| 场景 | 旧行为 | 新行为 |
|---|---|---|
| 部分批次早于 updateMessagesSent 消费 | 早到批次计数丢失，依赖 PowerJob 重试 | 早到批次在 RUNNING 下正常计数，后续批次继续累加，正常触发 |
| 全部批次早于 updateMessagesSent 消费 | completed_batch_count=N 但无人触发 SUCCESS | Producer 补偿检查命中，直接置 SUCCESS |

**解决方案：方案一（Producer 补偿）+ 方案二（放宽 WHERE 条件）**

**方案二（Consumer 侧改动）**：

将 WHERE 条件放宽为 `status IN ('RUNNING', 'MESSAGES_SENT')`，SUCCESS 判断改为直接引用 DB 列 `batch_count`，并加 `batch_count > 0` 守卫防止 RUNNING 阶段（此时 `batch_count=0`）被误触发：

```sql
-- CASE WHEN 的触发条件只引用 DB 列，不再需要传入 batchCount 参数
WHEN completed_batch_count + 1 >= batch_count AND batch_count > 0 THEN 'SUCCESS'
```

`batch_count > 0` 守卫的作用：RUNNING 阶段 `batch_count=0`，此时 `completed_batch_count + 1 >= 0` 永远为真，若无守卫会误置 SUCCESS。

**方案一（Producer 侧补偿）**：

`updateMessagesSent` 写入 `batch_count=N` 后，立即补查一次：若 `completed_batch_count >= batch_count` 已成立（说明所有批次在此之前已消费完），直接置 SUCCESS：
...
**会从第4批续拉。当前实现通过 `sync_batch_record` 支持批次级断点续拉。**

具体发生的事情：

1. 前3批（batch 0~2）已成功发到 MQ，`sync_batch_record` 记录 `pull_status=SUCCESS`；Consumer 正常写库
2. 第4批（batch 3）`client.pull()` 抛异常 → `syncBatchRecordService.markPullFailed(domain, date, 3, startRank, err)` → `sync_task_record` → **FAILED**；`MESSAGES_SENT` 未设置，`batchCount` 未记录
3. 前3批消费端调用 `incrementCompletedBatch`，但 WHERE 不匹配（status=FAILED）→ 计数不推进，任务不会变 SUCCESS
4. PowerJob 收到 `ProcessResult(false, ...)` → 触发任务级重试

**重跑过程（runRetrySync）**：
- `createOrUpdateRunning()` 重置为 RUNNING
- `hasSuccessBatch()=true`（batch 0~2 有 `pull_status=SUCCESS` 记录）→ 走 `runRetrySync()`
- `findFailed()` 返回 batch 3（`pull_status=FAILED`）
- 从 `batch3.startRank` 开始 while 循环重拉，覆盖 batch 3~7（**无 DELETE**）
- batch 0~2 数据保留；INSERT IGNORE 处理老 MQ 消息的重复消费
- batch 3~7 全部消费成功 → SUCCESS ✓

**注意**：`pull_status=FAILED` 类型的续拉会从失败批次的 `startRank` 一直拉到末尾（while 循环），因为 pull 中断意味着后续批次均未拉取。与之对比，`insert_status=FAILED` 只重拉单批（见下一题）。
...
**pull 循环不感知消费侧结果，8批全部发出；MQ 重试兜底；PowerJob 重跑时通过 `sync_batch_record` 仅补跑第5批，不删除其他7批数据。**

详细过程：

1. pull 循环继续，8批全部发出，各批 `sync_batch_record` `pull_status=SUCCESS`；sync_task_record → **MESSAGES_SENT(8)**
2. Consumer 消费第5批时 `batchInsert` 抛异常 → 向 Broker 返回失败
3. RocketMQ 按指数退避重投，最多3次；其余7批独立消费，`markInsertSuccess()` ✓，正常写库，`incrementCompletedBatch` 正常推进
4. 第5批3次仍失败 → 进入 DLQ
5. `FaultDataDlqConsumer`：`syncBatchRecordService.markInsertFailed(domain, date, 5, err)` + `syncTaskRecordService.updateFailed()` → sync_task_record **FAILED**，触发告警钩子

**最终状态**：7批数据在库，batch 5（5000条）缺失；`sync_batch_record` batch 5 `insert_status=FAILED`

**PowerJob 重跑（runRetrySync）**：
- `hasSuccessBatch()=true` → `runRetrySync()`
- `findFailed()` 返回 batch 5（`insert_status=FAILED`）
- **仅重拉 batch 5**（单次 pull from `startRank`，无 DELETE，无全量重跑）
- Consumer INSERT IGNORE 写入缺失5000条 → SUCCESS ✓
...
| sync_batch_record + runRetrySync | 持久批次失败；PowerJob 重跑时只补失败批次 | 最小，不动已成功数据 |
| DELETE + INSERT IGNORE | 首次全量同步的幂等兜底 | 有短暂空窗口（可接受） |

---

**Q（追问）：如果要求某批次失败后，保证其他批次已入库数据不被删除，只补跑失败批次，该如何设计？**

**A：**

**这正是当前实现的方案。** `sync_batch_record` 表 + `runRetrySync()` 已经实现了批次级补跑，可以展开说明其设计：

---

**核心机制一：`sync_batch_record` 表逐批记录 pull/insert 状态**

每批拉取和消费均在 `sync_batch_record` 留有记录（`pull_status` / `insert_status`，PENDING/SUCCESS/FAILED）：

- Pull 成功：`markPullSuccess(domain, date, batchIndex, startRank, endRank, count)`
- Pull 失败：`markPullFailed(domain, date, batchIndex, startRank, errMsg)` → 异常继续上抛
- Insert 成功（Consumer）：`markInsertSuccess(domain, date, batchIndex)`
- Insert 失败（DLQ Consumer）：`markInsertFailed(domain, date, batchIndex, errMsg)`

---

**核心机制二：`hasSuccessBatch()` 判断首次/重试路径**

`syncDomainDate()` 入口检查 `sync_batch_record` 中是否存在 `pull_status=SUCCESS` 的行：
- 无 → 首次运行 → `runFirstSync()`：DELETE + 全量拉取
- 有 → 重试运行 → `runRetrySync()`：不 DELETE，只补失败批次

---

**核心机制三：`runRetrySync()` 两种失败类型的差异化处理**

```
pull_status=FAILED（rank 游标中断）
  → findFailed() 返回该批次
  → 从 batch.startRank 开始 while 循环重拉到末尾
    （因为 pull 中断后，该批次及之后所有批次均未被拉取）

insert_status=FAILED（Consumer 写库失败）
  → findFailed() 返回该批次
  → 仅重拉该单批（单次 pull from startRank）
    （其他批次的数据已在 DB 中，无需重拉）
```

两种情况都不执行 DELETE，利用 INSERT IGNORE 处理可能的重复消费。

---

**权衡说明**

| 维度 | 首次运行（runFirstSync） | 重试运行（runRetrySync） |
|------|----------------------|----------------------|
| 是否 DELETE | 是（全量覆盖） | 否（保留已成功批次） |
| 重拉范围 | 全部批次 | 仅失败批次 |
| 幂等方式 | DELETE + INSERT IGNORE | INSERT IGNORE（无 DELETE） |
| 适用场景 | 从未同步过；或上游数据有修改需全量刷新 | 局部批次失败的精确修复 |

**注意**：重试路径跳过了已成功批次的重拉，若上游在已同步的 rank 范围内发生了数据修改，这些修改不会被感知。若业务要求完整性保障，可在修复后再触发一次首次同步（手动清空 `sync_batch_record` 即可重置为首次路径）。

---

**Q（追问）：若第二批次消费失败、第三批次消费成功，third 批次数据入库后 `last_successful_rank` 应如何更新？直接更新为第三批次的 maxRank 是否会导致第二批次被跳过？**

**A：**

**这是一个真实的设计陷阱，根本原因在于 `last_successful_rank` 是标量，只能表示连续进度，而 Consumer 的消费是并行且非连续的。**

**`last_successful_rank` 的正确职责边界**

`last_successful_rank` 只应由 **Producer 侧**在每批 pull + MQ send 成功后更新，用来记录"我已经把哪个 rank 之前的数据都发到 MQ 了"。

- pull 循环是顺序执行的：批次 1 → 批次 2 → 批次 3 ...
- 若批次 2 的 `client.pull()` 抛异常，循环立即终止，批次 3 **永远不会被拉取**
- 此时 `last_successful_rank` = 批次 1 的 maxRank，完全准确，没有间隙

所以 **Producer 侧的进度天然是连续的**，用一个标量字段追踪完全合理，不会出现间隙。

**间隙陷阱：Consumer 侧绝对不能用 `last_successful_rank` 追踪**

Consumer 侧消费是**并行且无序的**。假设批次 1、2、3 均已被 Producer 发出：

```
批次 1 Consumer → 成功入库 ✓
批次 2 Consumer → 失败 → 重试 → DLQ ✗
批次 3 Consumer → 成功入库 ✓
```

若用 `last_successful_rank` 追踪 Consumer 侧进度：

- 批次 3 成功 → 更新 `last_successful_rank` = max(批次3.rank)
- 批次 2 失败 → `last_successful_rank` 已越过批次 2 的范围
- 重跑时从 `last_successful_rank + 1` 开始 → **批次 2 的 rank 范围永远不会被补跑**

这就是间隙问题。

**正确解法：Consumer 侧失败用 `sync_batch_record` 按批次独立记录**

Consumer 侧的失败不能靠标量字段，需要一张按批次独立记录状态的表（已在当前代码中实现）：
...
CREATE TABLE sync_batch_record (
    ...
    start_rank    BIGINT       NOT NULL,               -- pull 起始游标
    end_rank      BIGINT       NOT NULL DEFAULT 0,     -- pull 成功后回填的最大 rank
...
DLQ Consumer 收到 batch 2 的死信后，调用 `markInsertFailed(domain, date, batchIndex=2, err)` 将 `insert_status=FAILED` 记录该行。`runRetrySync()` 通过 `findFailed()` 扫描 `insert_status=FAILED` 的行，只重拉 batch 2，batch 3 成功与 batch 2 失败完全独立，互不干扰。

**两种进度追踪的职责分离**

| 机制 | 追踪目标 | 更新方 | 数据结构 | 原因 |
|---|---|---|---|---|
| `last_successful_rank`（标量） | Producer 已发出的最高 rank | Producer（每批 send 成功后） | 单字段 | pull 循环顺序执行，进度天然连续 |
| `sync_batch_record`（表） | 每个批次的 Consumer 消费状态 | Consumer / DLQ Consumer | 每批一行 | 消费并行无序，必须按批次独立记录 |

**结论**：`last_successful_rank` 只能追踪 Producer 侧的连续拉取进度，Consumer 侧的非连续失败必须用按批次独立记录的补偿表。两套机制各司其职，不可混用。

---

**Q（追问）：sync_batch_record 记录了失败批次，那修复时如何只补跑第三批次而不影响其他批次的已入库数据？**

**A：**

答案分两层，第一层最简单，往往被忽视。

**第一层：DLQ 消息本身携带了数据，优先从消息直接重试**

`FaultDataBatchMessage` 消息体里包含了该批次的全部 records。消息进入 DLQ 时数据还在消息里，不需要重新去上游拉取。DLQ Consumer 的第一步应当直接用消息数据重试写库：

```
DLQ Consumer 收到 batch_2 的死信消息
    ├── markInsertFailed(domain, date, batchIndex=2, err) → sync_batch_record insert_status=FAILED
    ├── updateFailed(domain, date, err)  → sync_task_record = FAILED，触发告警
    └── PowerJob 重跑 → runRetrySync() → findFailed() 返回 batch_2
          → 重拉 batch_2（单次 pull from start_rank）→ 写库 → markInsertSuccess ✓ → SUCCESS
```

**第二层：runRetrySync 用 start_rank 定向重拉（当前已实现）**

`sync_batch_record` 记录了 `start_rank`，`runRetrySync()` 直接利用它从上游重拉该批次数据：

```java
// 查出 insert_status=FAILED 行：startRank=10001
List<FaultRecordDTO> records = sourceClient.pull(domain, date, batch.getStartRank(), pageSize);
...
// Consumer INSERT IGNORE 写入；markInsertSuccess 后 completedBatchCount 推进 → SUCCESS
```

其他批次数据完全不受影响，因为只重拉了 batch.startRank 起的单批数据，INSERT IGNORE 保证重复投递安全。

**完整数据流示例**

```
正常发送（5批）：
  Producer: batch_0(startRank=0,   endRank=5000)
            batch_1(startRank=5000, endRank=10000)
            batch_2(startRank=10000,endRank=15000)
            batch_3(startRank=15000,endRank=20000)
            batch_4(startRank=20000,endRank=22000)
  updateMessagesSent(batchCount=5) + checkAndMarkSuccessIfAllDone

消费阶段：
  batch_0 → OK  markInsertSuccess  completedBatchCount=1
  batch_1 → OK  markInsertSuccess  completedBatchCount=2
  batch_2 → 失败×3 → DLQ
  batch_3 → OK  markInsertSuccess  completedBatchCount=3
  batch_4 → OK  markInsertSuccess  completedBatchCount=4

DLQ Consumer：
  markInsertFailed(domain, date, batchIndex=2, err)
  updateFailed(domain, date, err)  → sync_task_record = FAILED，触发告警

PowerJob 重跑 → runRetrySync()：
  findFailed() → batch_2（insert_status=FAILED，startRank=10000）
  pull(domain, date, startRank=10000, 5000) → rank 10001~15000
  sendBatch(batchIndex=2, startRank=10000, records)
  Consumer INSERT IGNORE 写入 batch_2
  markInsertSuccess(batchIndex=2) → completedBatchCount=5 → SUCCESS ✓
```

**与全量重跑的对比**

| 维度 | 全量重跑（首次路径） | 批次级修复（重试路径） |
|---|---|---|
| 需要重拉的数据量 | 全部5批（22000条） | 仅 batch_2（5000条） |
| 其他批次已入库数据 | DELETE 清空后重写 | 完全不动 |
| 是否执行 DELETE | 是 | 否 |
| 前提 | 无 | 上游数据 rank 稳定，不乱序修改 |
