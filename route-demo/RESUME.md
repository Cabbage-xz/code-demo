...
### 3.6 常见误区辨析：这次重构是策略、模板还是工厂模式？

面试中容易被追问："你把 6 套代码合并成 1 套，用的是哪种设计模式？"

**三者为何都不准确**：

| 模式 | 解决的问题 | 为何不适用 |
|------|-----------|-----------|
| **策略模式** | 同一行为有多种算法实现，运行时选一种 | 6 个维度的算法完全相同，没有"多种算法"可选 |
| **模板方法** | 流程相同，部分步骤由子类差异化实现 | 没有任何步骤需要子类 override，全程一套逻辑 |
| **工厂模式** | 需要创建不同类型的对象 | 重构后根本不需要多个对象，只有一套 Bean |

**根本原因**：这次重构识别到 6 套代码的**唯一变化点是表名**（一个字符串），而非算法、行为或对象类型。GoF 行为模式的前提是"行为存在差异"，但这里差异压根不在行为层面。

**正确描述**：这是一次**参数化重构**——将变化点从类结构（6 个类）外化为运行时数据（yml 配置 + ThreadLocal），是**开闭原则（OCP）的直接落地**：新增维度只加配置，不动代码。实现机制对应**装饰器模式**（`DynamicTableNameInnerInterceptor` 透明增强 SQL）。

**面试标准答案**：
> "这次重构本身不对应某个具体的 GoF 行为模式。核心洞察是 6 套重复代码的差异点只有表名一个配置项，所以正确的抽象不是 OO 多态（多个实现类），而是将变化外化为数据。实现层面用的是装饰器模式——`DynamicTableNameInnerInterceptor` 在不改变 Mapper 接口的前提下透明替换 SQL 中的表名，这是装饰器'增强而不修改接口'的标准用法。"
